...
$(TSDIR):
	...
	@cd $(TSDIR) && npm --loglevel=info --progress=true install && npm run build

.PHONY: parse-% extract-tests
extract-tests: dev
	@cd $(TSDIR)/corpus && find . -type f -name "*.txt" -print0 |    \
		while IFS= read -r -d '' f; do                           \
		ff="$$(basename $$f)";                                   \
		$(BINDIR)/examples.rb < $$f > $(TESTDIR)/$${ff%.*}.scss; \
	done
